...
public partial class AutoCompleteView : ContentView
{
    ...
    /// </summary>
    public static readonly BindableProperty SearchBackgroundColorProperty = BindableProperty.Create(nameof(SearchBackgroundColor), typeof(Color),
                                                                                                    typeof(AutoCompleteView), Colors.White, BindingMode.TwoWay, propertyChanged: SearchBackgroundColorChanged);
    ...
    public static readonly BindableProperty LineColorProperty =
        BindableProperty.CreateAttached("LineColor", typeof(Color), typeof(AutoCompleteView), Colors.Black);
    ...
    private static void TextValueChanged(BindableObject obj, object oldVal, object newVal)
    {
        ...
        if (obj is AutoCompleteView control)
        {
            ...
            var cleanedNewPlaceHolderValue = ReplaceRegex().Replace((newPlaceHolderValue ?? string.Empty).ToLowerInvariant(), string.Empty);
            ...
            if (!string.IsNullOrEmpty(cleanedNewPlaceHolderValue) && control.Suggestions != null)
            {
                var filteredSuggestions = control.Suggestions.Cast<object>()
                    .Where(x => ReplaceRegex().Replace(x.ToString().ToLowerInvariant(), string.Empty).Contains(cleanedNewPlaceHolderValue))
                    .OrderByDescending(x => ReplaceRegex().Replace(x.ToString()
